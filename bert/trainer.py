...
def percentage(batch_size: int, max_index: int, current_index: int):
    """ Calulate epoch progress percentage """
...
def nsp_accuracy(result: torch.Tensor, target: torch.Tensor):
    """ Calculate the accuraxy of NSP between two tensors """  
...
def token_accuracy(result: torch.Tensor, target: torch.Tensor, inverse_token_mask: torch.Tensor):
    ...
    r = result.argmax(-1).masked_select(~inverse_token_mask.bool())
    t = target.masked_select(~inverse_token_mask.bool())
    s = (r == t).sum()
...
class BertTrainer:
    ...
    def train(self, epoch: int):
        ...
        for i, value in enumerate(self.loader):
            ...
            token = token.masked_fill(tm.bool(), 0)
    ...
    def training_summary(self, elapsed, index, average_nsp_loss, average_mlm_loss):
        ...
        s += f" | Epoch: {self.current_epoch} | {index} / {self._batched_len} ({passed}%) | " \
    ...
    def save_checkpoint(self, epoch, step, loss):
        ...
        prev = time.time()
        name = f"bert-{epoch}-{step}-{loss}-{datetime.utcnow().timestamp():.0f}.pt"
        ...
        print()
        print("=" * self._splitter_size)
        print(f"Model saved as '{name}' for {time.time() - prev:.2f} seconds")
        print("=" * self._splitter_size)
        print()
